...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    ...
    go build -ldflags "-X github.com/Toinounet21/avalanchego-mod/version.GitCommit=$git_commit -X github.com/Toinounet21/coreth-mod/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
else
    echo "Building AvalancheGo with rocksdb enabled..."
    go build -tags rocksdballowed -ldflags "-X github.com/Toinounet21/avalanchego-mod/version.GitCommit=$git_commit $static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
fi
