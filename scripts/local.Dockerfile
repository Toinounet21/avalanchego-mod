...
RUN mkdir -p /go/src/github.com/Toinounet21

WORKDIR $GOPATH/src/github.com/Toinounet21
COPY avalanchego avalanchego
COPY coreth coreth

WORKDIR $GOPATH/src/github.com/Toinounet21/avalanchego
RUN ./scripts/build_avalanche.sh
RUN ./scripts/build_coreth.sh ../coreth $PWD/build/plugins/evm

RUN ln -sv $GOPATH/src/github.com/Toinounet21/avalanche-byzantine/ /avalanchego
